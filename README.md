...
部署到服务器 / 开机自启 / 关联图 / 多机同步：见下方链接。

### English quickstart

Libry is an open-source, self-hostable personal knowledge base maintained by AI agents. The vault is plain markdown (Obsidian-compatible); the engine is a small Python package.

```bash
git clone https://github.com/CobyLee66/Libry.git ~/Libry
cd ~/Libry && python3.11 -m venv .venv && .venv/bin/pip install .
.venv/bin/libry init ~/my-kb            # scaffold vault, set admin password
cd ~/my-kb && ~/Libry/.venv/bin/libry serve   # http://127.0.0.1:8000
```

Drop your coding agent (Claude Code / hermes / any AGENTS.md-aware CLI) into the vault — it will discover `AGENTS.md` and `.agents/skills/` and start ingesting. Docs (mostly Chinese): [docs/](docs/).
